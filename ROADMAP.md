...
- **Test suite** — 68 unit tests covering Anthropic + OpenAI-compatible (SDK and raw HTTP) judges, runner cache hit/miss, cache invalidation per input, atomic write, and corrupt-file recovery; all clients fully mocked.
...
1. **Diff mode (`--changed-only` against a base ref).** Skip files unchanged since a base ref. Composes with the shipped persistent verdict cache (which keys on `file_content_sha256` and so already drops cache hits to ~free). Diff mode is the cheaper *bypass* — it doesn't even read unchanged files — and matters most on huge repos where the cache lookup overhead itself adds up. They compose: ship both.
2. **GitHub Actions annotations.** Emit `::error file=X,line=Y,title=...::reason` so failures surface as inline PR comments instead of buried in a log. Add as a `--format github` flag; near-zero implementation cost; large UX win for the marketed v1.
3. **Cost / token estimation (`docs-ci estimate`).** Use `count_tokens` to predict cost before running. Useful when adding rules to a large doc set. The LLM-judge model is cheap but not free; surfacing the cost upfront builds trust and prevents surprise bills.

### Inference provider abstraction (continued)

4. **Provider ergonomics.** `docs-ci providers test [NAME]` for pre-flight auth + model validation (one cheap call to verify env vars and model IDs before paying for a real scan), and a generic `--provider openai-compat --base-url ... --api-key-env ...` escape hatch for OpenAI-compatible endpoints we don't curate by name (Together, Groq, DeepSeek, OpenAI direct, local Ollama / vLLM, etc.). A project-local `.docs-ci.yaml` with named provider profiles is deferred until item 5 (per-rule overrides) needs a place to resolve names from. Skipping `providers add` / `remove` subcommands — editing YAML is fine.
