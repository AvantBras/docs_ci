...
- **`action.yml` wrapper** — composite GitHub Action at the repo root. Consumers `uses: AvantBras/docs_ci@v1` and pass inputs (`path`, `rules`, `api-key`, etc.) instead of writing install / setup-python / invoke YAML by hand. Inputs map 1:1 to CLI flags; `format` defaults to `github` (vs `text` on the CLI) since CI is the action's only surface. `api-key` is optional with an env-var fallback so externally-set provider keys still work. Linux-only in v0.
...
The biggest gap right now is that docs_ci has only been smoke-tested against synthetic fixtures. Before committing to the priorities below, the plan is to run it against 1–3 real documentation sets and see what breaks, what surprises, and where the judge gets confused on actual prose. With the action wrapper shipped, dropping docs_ci into a target repo is now a 6-line workflow rather than a hand-rolled install/invoke chain — so this work is unblocked.
...
1. **Rule self-tests.** A rule declares known-pass and known-fail fixture files; `docs-ci test-rules` verifies the LLM still calls them right when models or prompts change. For a system where prose *is* the spec, this is the meta-eval harness — the only defense against silent regression on a model upgrade. Promoted from "exploratory" because nothing else replaces it.
2. **Cost / token estimation (`docs-ci estimate`).** Predict cost before running. Useful when adding rules to a large doc set. Caveat: Anthropic's `count_tokens` is provider-specific — OpenRouter and NVIDIA don't expose an equivalent. v0 of this feature is Anthropic-only with a clear "not supported on provider X" message; a generic tokenizer-based estimate is a follow-up.
3. **Per-rule `include` / `exclude` globs.** Already deferred in [AGENTS.md](AGENTS.md). Independently useful as soon as real-docs testing hits projects with mixed content (API docs, blog posts, changelogs that shouldn't all be judged the same way).
4. **Provider ergonomics.** A generic `--provider openai-compat --base-url ... --api-key-env ...` escape hatch for OpenAI-compatible endpoints we don't curate by name (Together, Groq, DeepSeek, OpenAI direct, local Ollama / vLLM, etc.). The escape hatch is the meatier half — it unblocks every provider not curated by name. Plus a `docs-ci providers test [NAME]` pre-flight (one cheap call to verify env vars and model IDs before paying for a real scan). A project-local `.docs-ci.yaml` with named provider profiles is deferred until item 5 needs a place to resolve names from.
